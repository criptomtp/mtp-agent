interface Lead {
  ...
  score?: number;
  score_grade?: string;
  analysis_json?: { score?: number; grade?: string };
  created_at: string;
}
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody>
          {leads.map((lead) => {
            const score = lead.score ?? lead.analysis_json?.score ?? 0;
            return (
              <tr
                key={lead.id}
              ...
              >
                ...
                <td className="py-2">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium ${getScoreColor(score)}`}
                  >
                    {score}/10
                  </span>
                </td>
